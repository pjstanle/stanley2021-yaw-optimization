...
def power_objective(input_dict):
    ...
    farm_power = floris_model.get_farm_power()

    funcs["power_obj"] = -farm_power/power_scale
...
if __name__=="__main__":
    ...
    for n in range(len(wd)):
        ...
        power_scale = base_power
        opt = "SNOPT"

        start_yaw = np.zeros(nturbs)
    
        optProb = pyoptsparse.Optimization("yaw_optimization",power_objective)
        ...
        optProb.addObj("power_obj")
        solution = optimize(optProb,sens="FD")

        opt_power = -solution.objectives["power_obj"].value * power_scale
        optimized_aep += opt_power*8760.0*wf[n]
        

        baseline_power_array[n] = base_power
        optimized_power_array[n] = opt_power
        percent_improvement_array[n] = (opt_power-base_power)/base_power*100.0
    ...
    progress_filename = "amalia_continuous"
