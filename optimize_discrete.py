...
if __name__=="__main__":
    ...
    seed_array = [0,1,2,3,4,5,6]
    yaw_angle_array = [5,10,15,25,30]

    for n in range(len(seed_array)):
        for m in range(len(yaw_angle_array)):
            random_seed = seed_array[n]
            np.random.seed(random_seed)
            yaw_angle = yaw_angle_array[m]
            progress_filename = "random_layout/discrete/discrete_seed%s_%syaw"%(random_seed,yaw_angle)
            ...
            for k in range(len(nturbs_array)):
                nturbs = nturbs_array[k]
                # turbine_x = np.linspace(0,240*5*(nturbs-1),nturbs)
                # turbine_y = np.zeros(nturbs)
                spacing = 240*5
                side = (np.sqrt(nturbs)-1)*spacing
                turbine_x, turbine_y = place_turbines(nturbs,side,240.0,240.0*3.0)
